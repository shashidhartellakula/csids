from flask import send_file, Flask, render_template, request, redirect, url_for, flash, jsonify, abort
import io, os
...
def dashboard():
    ...
    recent = cur.fetchall()
    cur.execute("SELECT strftime('%H', timestamp) as hour, COUNT(*) as cnt FROM alerts GROUP BY hour ORDER BY hour")
    hourly_raw = cur.fetchall()
    cur.execute("""
        SELECT CASE WHEN risk_score>=6 THEN 'High' WHEN risk_score>=3 THEN 'Medium' ELSE 'Low' END as level, COUNT(*) as cnt
        FROM alerts GROUP BY level
    """)
    risk_raw = cur.fetchall()
    cur.execute("SELECT id, username, role, email, created_at FROM auth_users ORDER BY created_at DESC")
    ...
    return render_template("dashboard.html",
        total_users=total_users, total_alerts=total_alerts, alerts_24h=alerts_24h,
        top_users=top_users, recent_alerts=recent, hourly=hourly,
...
def admin_send_alert():
    ...
    conn = get_db()
    cur  = conn.cursor()
    cur.execute("SELECT * FROM alerts WHERE user = ? ORDER BY timestamp DESC LIMIT 10", (target_user,))
...
def admin_user_detail(username):
    conn = get_db()
    cur  = conn.cursor()
    cur.execute("SELECT * FROM alerts WHERE user=? ORDER BY timestamp DESC LIMIT 200", (username,))
    alerts = cur.fetchall()
    cur.execute("SELECT * FROM live_log WHERE user=? ORDER BY timestamp DESC LIMIT 200", (username,))
    commands = cur.fetchall()
    cur.execute("SELECT sequence, frequency FROM user_sequences WHERE user=? ORDER BY frequency DESC LIMIT 50", (username,))
    sequences = cur.fetchall()
    conn.close()
    stats = get_profile_stats(username)
    return render_template("admin_user_detail.html",
        username=username, alerts=alerts, commands=commands,
        sequences=sequences, stats=stats)
...
def analyze():
    if request.method == "POST":
        ...
        if alerts:
            ...
            cur  = conn.cursor()
            for a in alerts:
                cur.execute("INSERT INTO alerts (user,sequence,reason,risk_score,risky_cmds) VALUES (?,?,?,?,?)",
                    (user, a["sequence"], a["reason"], a["risk_score"], ",".join(a["risky"])))
            conn.commit()
            conn.close()
            if notify and email:
                from notifier import send_alert_email
                send_alert_email(email, user, alerts)
        return render_template("results.html", alerts=alerts, user=user, total=len(sequences))
...
@admin_required
def live_monitor():
    # pass all registered users for admin to pick from
    conn = get_db()
    cur  = conn.cursor()
    cur.execute("SELECT username FROM auth_users WHERE role='user' ORDER BY username")
    user_list = [r["username"] for r in cur.fetchall()]
    conn.close()
    return render_template("live.html", locked_user=None, user_list=user_list)
...
def settings():
    ...
    return render_template("settings.html", smtp_host=smtp_host, smtp_port=smtp_port, smtp_user=smtp_user)
...
def download_report(username):
    ...
    conn = get_db()
    cur  = conn.cursor()
    cur.execute("SELECT * FROM alerts WHERE user=? ORDER BY timestamp DESC LIMIT 500", (username,))
    ...
    return send_file(io.BytesIO(pdf_bytes), mimetype="application/pdf",
        as_attachment=True, download_name=f"csids_report_{username}.pdf")
...
def user_dashboard():
    ...
    conn     = get_db()
    cur      = conn.cursor()
    cur.execute("SELECT COUNT(*) FROM alerts WHERE user=?", (username,))
    total_alerts = cur.fetchone()[0]
    cur.execute("SELECT COUNT(*) FROM alerts WHERE user=? AND timestamp > datetime('now','-1 day')", (username,))
    alerts_24h = cur.fetchone()[0]
    cur.execute("SELECT * FROM alerts WHERE user=? ORDER BY timestamp DESC LIMIT 8", (username,))
    recent = cur.fetchall()
    cur.execute("""
        SELECT CASE WHEN risk_score>=6 THEN 'High' WHEN risk_score>=3 THEN 'Medium' ELSE 'Low' END as level, COUNT(*) as cnt
...
def user_upload():
    ...
    if request.method == "POST":
        ...
        if alerts:
            ...
            for a in alerts:
                cur.execute("INSERT INTO alerts (user,sequence,reason,risk_score,risky_cmds) VALUES (?,?,?,?,?)",
                    (username, a["sequence"], a["reason"], a["risk_score"], ",".join(a["risky"])))
            conn.commit()
            conn.close()
            if notify and current_user.email:
                from notifier import send_alert_email
                send_alert_email(current_user.email, username, alerts)
        return render_template("results.html", alerts=alerts, user=username, total=len(sequences))
...
def user_alerts():
    ...
    conn = get_db()
    cur  = conn.cursor()
    cur.execute("SELECT * FROM alerts WHERE user=? ORDER BY timestamp DESC LIMIT 200", (current_user.username,))
    alerts = cur.fetchall()
    conn.close()
...
def api_live_log():
    ...
    if not current_user.is_admin:
        user = current_user.username          # force own data only
    conn = get_db()
    cur  = conn.cursor()
    if user:
        cur.execute("SELECT * FROM live_log WHERE id>? AND user=? ORDER BY id ASC LIMIT 50", (since_id, user))
    else:
        cur.execute("SELECT * FROM live_log WHERE id>? ORDER BY id ASC LIMIT 50", (since_id,))
    rows = [dict(r) for r in cur.fetchall()]
    conn.close()
    return jsonify(rows)
...
def api_recent_alerts():
    ...
    cur  = conn.cursor()
    if user:
        cur.execute("SELECT * FROM alerts WHERE id>? AND user=? ORDER BY id ASC LIMIT 20", (since_id, user))
    else:
        cur.execute("SELECT * FROM alerts WHERE id>? ORDER BY id ASC LIMIT 20", (since_id,))
    rows = [dict(r) for r in cur.fetchall()]
    conn.close()
    return jsonify(rows)
